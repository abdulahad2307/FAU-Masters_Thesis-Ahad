...
from utils.class_IL.dataloader_utils import get_class_il_loader, EAMLClassILDataset, common_transform, eaml_collate_fn
...
from utils.class_IL.cil_utils import (
    StandardIncremental, DistillationIncremental, EWC, ExemplarManager, AdaptiveLR, extract_features, extract_features_and_logits
)
...
def run_incremental_learning_ood(
    data_root: str,
    ocr_tensor_path: str,
    ...
    # --- Validating unseen_classes list ---
    ...
    assert ood_method.lower() in ood_methods, f"OOD method '{ood_method}' is not implemented."
    OODClass = ood_methods[ood_method.lower()]
    ...
    for unseen_idx, new_class in enumerate(unseen_classes[start_unseen_index:], start=start_unseen_index):
        ...
        if len(previous_classes) > 0:
            ...
            if prev_ckpt is not None:
                old_model.load_state_dict(prev_ckpt.get("model_state_dict", prev_ckpt), strict=False)
                del prev_ckpt
                torch.cuda.empty_cache()
            old_model.eval()
        ...
        for epoch in range(this_start_epoch, num_epochs):
            ...
            if val_result['top1_acc'] > this_step_best_acc:
                ...
                # -------- OOD method evaluation -------
                print(f"Running OOD ({ood_method.upper()}) open set evaluation on validation set...")

                if ood_method.lower() == "vim":
                    features_by_class = {}
                    logits_by_class = {}
                    for cls in current_classes:
                        class_loader = get_class_il_loader(model_name, os.path.join(data_root, "train"), [cls], batch_size, ocr_data=ocr_tensor_path)
                        feats, logs = extract_features_and_logits(model, class_loader, DEVICE)
                        features_by_class[cls] = feats
                        logits_by_class[cls] = logs
                    ood_detector.fit(features_by_class, logits_by_class)
                else:
                    ...
                    ood_detector.fit(features_by_class)

                val_features_dict = extract_features(model, val_loader, DEVICE)
                ood_score_res = ood_detector.ood_metrics(val_features_dict, known_classes=current_classes)
                print(f"[OOD-{ood_method.upper()} Open Set Val] Known acc: {ood_score_res['open_set_accuracy']:.4f}, Unknown rejection: {ood_score_res['unknown_rejection']:.4f}")
            else:
    ...
    if final_global_best_path:
        ...
        # --- OOD open set test evaluation ---
        test_features_dict = extract_features(final_model, test_loader_full, DEVICE)
        ood_score_res = ood_detector.ood_metrics(test_features_dict, known_classes=all_classes)
        print(f"[OOD-{ood_method.upper()} Open Set Test] Known acc: {ood_score_res['open_set_accuracy']:.4f}, Unknown rejection: {ood_score_res['unknown_rejection']:.4f}")

        savepath = os.path.join(checkpoint_dir, f"ood_{ood_method}_hist.png")
        if ood_score_res.get("y_true") is not None and ood_score_res.get("scores") is not None:
            plot_ood_histograms(
                ood_score_res["y_true"],
                ood_score_res["y_pred"],
                ood_score_res["scores"],
                savepath=savepath
            )
