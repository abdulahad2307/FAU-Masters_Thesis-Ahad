...
def normalize_bbox_layoutlm(bbox, img_width, img_height):
    """Normalize bbox [x0, y0, x1, y1] to 1000-based scale expected by LayoutLMv3."""
    return [
...
def normalize_bbox_polygon8(bbox8, img_width, img_height):
    """Normalize an 8-point polygon (x1,y1,x2,y2,x3,y3,x4,y4) to 0..1 for DocFormer style."""
    return [b / img_width if i % 2 == 0 else b / img_height for i, b in enumerate(bbox8)]

def polygon8_to_rect4(bbox_8):
    """From polygon [x1,y1,x2,y2,x3,y3,x4,y4] to rect [x_min,y_min,x_max,y_max]"""
    xs = bbox_8[0::2]
    ys = bbox_8[1::2]
...
def tokenize_and_save_layoutlm(texts, bboxes_rect, bboxes_poly, tokenizer, img_size, max_len, output_path, out_bbox_style="rect"):
    ...
    # Insert [CLS] and [SEP] at head and tail
    words = ["[CLS]"] + words + ["[SEP]"]
    boxes = [special_box] + boxes + [special_box]

    # Truncate to max_len (keeps [CLS], main tokens, [SEP])
    words = words[:max_len]
    boxes = boxes[:max_len]

    # Pad up to max_len with [PAD] and dummy box
    pad_len = max_len - len(words)
    if pad_len > 0:
        words += ["[PAD]"] * pad_len
        boxes += [special_box] * pad_len

    # Tokenizer call: pre-tokenized mode (correct argument is text=words)
    encoding = tokenizer(
        text=words,
        boxes=boxes,
        padding="max_length",
...
def precompute_ocr_layoutlm(data_dir, output_dir, ocr_engine="tesseract", max_size=1024, max_len=512, offset=0, max_images=None, bbox_style="rect"):
    ...
    for img_path in tqdm(all_image_paths):
        try:
            ...
            save_path = os.path.join(output_dir, os.path.splitext(os.path.basename(img_path))[0] + ".pt")
            tokenize_and_save_layoutlm(texts, bboxes_rect, bboxes_poly,
                                       tokenizer, image.size, max_len, save_path, bbox_style)
            processed += 1
            del image
            gc.collect()
        except Exception as e:
