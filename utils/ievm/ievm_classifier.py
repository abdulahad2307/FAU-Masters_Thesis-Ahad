...
class IncrementalEVM:
    ...
    PyTorch version based on the TensorFlow-iEVM (official repo).
    Usage mirrors scikit-learn style: fit, incremental_update, predict_proba, predict.
    """
    ...
    def __init__(self, tailsize=0.3, ev_budget=10, cover_threshold=0.7, distance_metric="euclidean"):
        ...
        self.class_evs = {}    # {class: [ev_tuple, ...]; ev_tuple=(vector, scale, shape, max_tail_dist)}
        self.class_features = {}  # {class: np.ndarray}
    ...
    def fit_weibull(self, point, negatives):
        ...
        # Catch for rare degenerate cases:
        try:
    ...
    def incremental_update(self, new_features_dict):
        ...
        for label, feats in new_features_dict.items():
            ...
            # Refit only EVs within max_tail, plus any new samples
            new_evs = []
    ...
    def predict_proba(self, features):
        ...
        for label, evs in self.class_evs.items():
            ...
            results[label] = 1 - scores  # Return higher value for closer samples
        return results
